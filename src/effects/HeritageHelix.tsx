...
export default function HeritageHelix() {
  ...
  useEffect(() => {
    ...
    const chars = ['WEB', 'AI', 'CRM', 'DATA', 'BRAND', 'CODE', '|', '-', '+', '/', '\\', '<', '>'];
    const totalRungs = 80;
    const radius = 120;
    const fov = 1000;
    ...
    function draw() {
      ...
      for (let i = 0; i < totalRungs; i++) {
        ...
        ctx!.strokeStyle = 'rgba(248, 101, 179, 0.12)';
        ctx!.lineWidth = 0.5;
        ctx!.stroke();
      }
      ...
      // Draw characters
      for (let idx = 0; idx < allParticles.length; idx++) {
        const p = allParticles[idx];
        const isMagenta = idx % 2 === 0;
        const color = isMagenta ? `rgba(248, 101, 179, ${p.alpha})` : `rgba(156, 246, 246, ${p.alpha})`;
  ...
  return (
    <div
      ...
      style={{
        ...
        width: '100%',
        height: '100%',
        background: 'radial-gradient(ellipse at center, rgba(1,22,39,0) 0%, #011627 75%)',
      }}
    >
      <canvas
        ref={canvasRef}
