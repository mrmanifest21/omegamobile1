...
const EMAIL_TO = 'omegaelz@outlook.com';
...
type SubmitType = 'whatsapp' | 'email' | null;

export default function Contact() {
  const [form, setForm] = useState({ name: '', email: '', company: '', service: '', budget: '', message: '' });
  const [submitted, setSubmitted] = useState<SubmitType>(null);
  const [hoveringWA, setHoveringWA] = useState(false);
  ...
  const buildEmailBody = () => [
    ...
    form.budget ? `Budget: ${form.budget}` : '',
    form.message ? `\nMessage:\n${form.message}` : '',
    `\n---\nSent via omegaelz.com contact form`,
  ].filter(Boolean).join('\n');

  const handleWhatsApp = (e: React.FormEvent) => {
    e.preventDefault();
    window.open(`https://wa.me/${WHATSAPP}?text=${encodeURIComponent(buildWhatsAppMsg())}`, '_blank', 'noopener,noreferrer');
  ...
  const contactDetails = [
    ...
    { label: 'Email', value: 'omegaelz@outlook.com', href: `mailto:${EMAIL_TO}` },
        ...
        <div className="contact-layout">
          ...
          <div>
            ...
            <p style={{ fontFamily: '"DM Sans", sans-serif', fontSize: '16px', color: 'rgba(216,226,220,0.5)', lineHeight: 1.7, maxWidth: '400px', marginBottom: '56px' }}>
              Fill in the form and choose how you'd like to reach us — WhatsApp for instant chat, or email straight to our inbox. We respond within 24 business hours.
            </p>
            ...
            {submitted ? (
              <div style={{ textAlign: 'center', padding: '48px 0' }}>
                <div style={{
                  ...
                  backgroundColor: submitted === 'whatsapp' ? 'rgba(37,211,102,0.08)' : 'rgba(248,101,179,0.08)',
                  border: `1px solid ${submitted === 'whatsapp' ? 'rgba(37,211,102,0.4)' : 'rgba(248,101,179,0.4)'}`,
                  ...
                  {submitted === 'whatsapp' ? (
                    ...
                    <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="#F865B3" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
                ...
                <h3 style={{ fontFamily: '"DM Sans", sans-serif', fontSize: '26px', color: '#D8E2DC', marginBottom: '12px', letterSpacing: '-0.02em' }}>
                  {submitted === 'whatsapp' ? 'WhatsApp Opened!' : 'Email Client Opened!'}
                </h3>
                <p style={{ fontFamily: '"Space Mono", monospace', fontSize: '12px', color: 'rgba(216,226,220,0.45)', lineHeight: 1.7 }}>
                  {submitted === 'whatsapp'
                    ? 'Your message is pre-filled in WhatsApp.\nJust hit send.'
                    : `Your enquiry is pre-filled and addressed\nto ${EMAIL_TO}. Just hit send.`}
                </p>
                <button
              ...
              <form style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                ...
                <div className="contact-form-grid-2">
                  ...
                  <FormInput label="Email Address" name="email" type="email" value={form.email} placeholder="john@company.com" onChange={handleChange as (e: React.ChangeEvent<HTMLInputElement>) => void} />
                </div>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                  <div style={{ flex: 1, height: '1px', backgroundColor: 'rgba(216,226,220,0.08)' }} />
                  <span style={{ fontFamily: '"Space Mono", monospace', fontSize: '9px', color: 'rgba(216,226,220,0.2)', letterSpacing: '0.1em', textTransform: 'uppercase' }}>Choose how to send</span>
                  <div style={{ flex: 1, height: '1px', backgroundColor: 'rgba(216,226,220,0.08)' }} />
                </div>

                {/* Dual CTA Row */}
                <div className="contact-send-btns">
                  {/* WhatsApp Button */}
                  <button
                    type="submit"
                    disabled={!isValid}
                    onClick={handleWhatsApp}
                    onMouseEnter={() => setHoveringWA(true)}
                    onMouseLeave={() => setHoveringWA(false)}
                    style={{
                      fontFamily: '"Space Mono", monospace', fontSize: '11px', fontWeight: 400,
                      color: '#011627', backgroundColor: !isValid ? 'rgba(37,211,102,0.3)' : hoveringWA ? '#1ebe5d' : '#25D366',
                      border: 'none', padding: '16px 12px', cursor: isValid ? 'pointer' : 'not-allowed',
                      letterSpacing: '0.05em', textTransform: 'uppercase',
                      display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px',
                      transition: 'background-color 0.3s ease, box-shadow 0.3s ease',
                      boxShadow: hoveringWA && isValid ? '0 0 30px rgba(37,211,102,0.3)' : 'none',
                  ...
                  <button
                    ...
                    style={{
                      fontFamily: '"Space Mono", monospace', fontSize: '11px', fontWeight: 400,
                      color: hoveringEmail && isValid ? '#011627' : '#F865B3',
                      backgroundColor: hoveringEmail && isValid ? '#F865B3' : 'transparent',
                      border: '1px solid rgba(248,101,179,0.4)',
                      padding: '16px 12px', cursor: isValid ? 'pointer' : 'not-allowed',
                      letterSpacing: '0.05em', textTransform: 'uppercase',
                      display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px',
                      transition: 'all 0.3s ease',
                      boxShadow: hoveringEmail && isValid ? '0 0 30px rgba(248,101,179,0.2)' : 'none',
                      opacity: isValid ? 1 : 0.5,
                    }}
                  >
                ...
                <p style={{ fontFamily: '"Space Mono", monospace', fontSize: '10px', color: 'rgba(216,226,220,0.2)', textAlign: 'center', lineHeight: 1.7 }}>
                  No spam. We respond within 24 business hours. Your data is protected under POPIA.
                </p>
              </form>
            )}
          </div>
